...
def ajouter_utilisateur(user_id, id_melbet):
    try:
        # On force user_id en int (ou str selon ta table, ici int pour du bigint)
...
def valider_utilisateur(user_id):
    try:
        supabase.table("users").update({"status": "active"}).eq("user_id", int(user_id)).execute()
    except Exception as e:
        print(f"Erreur Supabase (valider_utilisateur) : {e}")

def est_valide(user_id):
    try:
        response = supabase.table("users").select("status").eq("user_id", int(user_id)).execute()
        data = response.data
